...
spispok_papok = os.listdir(r'C:\Users\Denis\PycharmProjects\pythonProject\На чистку')
for papka in spispok_papok:
    for f in os.scandir(fr'C:\Users\Denis\PycharmProjects\pythonProject\На чистку\{papka}'):
        print(f'В работе {f}')
        tree = ET.parse(f)
        root = tree.getroot()
        proverka = False
        #Удаляем хмл, если в ней больше 4 комнат и где в адресcе есть СНТ или слово "казарма"
        adress = [i.text.split() for i in root.iter('readable_address')]
        for text in adress:
            for snt in text:
                if snt == "СНТ" or snt == 'казарма' or snt == "снт":
                    os.remove(fr'C:\Users\Denis\PycharmProjects\pythonProject\На чистку\{papka}\{f.name}')
...
df.to_excel(r'C:\Users\Denis\PycharmProjects\pythonProject\Данные из xml.xlsx', index= False)
